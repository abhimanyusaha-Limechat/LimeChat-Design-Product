/**
 * Icon set for the Broadcast home page (Figma node 7810:114718).
 *
...
const PATHS: Record<BroadcastIconName, string[]> = {
...
type IconProps = SVGProps<SVGSVGElement> & { name: BroadcastIconName };
...
export function BroadcastIcon({ name, className, ...props }: IconProps) {
  if (name === 'loader') {
    return (
      <svg
        className={`lc-bh__icon${className ? ` ${className}` : ''}`}
        data-icon={name}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
