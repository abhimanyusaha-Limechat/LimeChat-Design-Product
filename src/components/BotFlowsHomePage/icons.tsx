/**
 * Icon set for the Bot Flows home page (Automation → Flows) — copied from
 * SegmentsHomePage's icon set (same visual language) for scoping.
 *
...
const PATHS: Record<BotFlowIconName, string[]> = {
...
type IconProps = SVGProps<SVGSVGElement> & { name: BotFlowIconName };

export function BotFlowIcon({ name, className, ...props }: IconProps) {
  return (
    <svg
      className={`lc-bf__icon${className ? ` ${className}` : ''}`}
      data-icon={name}
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
